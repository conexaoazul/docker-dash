...
**Why deferred:** v6.6.0 ships entry points on Secrets Audit only. The `RemediateWizard.open()` component is reusable — adding buttons on other pages is mechanical 10-line edits per page. Not blocking the headline feature.

**Estimated effort:** 1-2 hours for all three pages.
**Proposed approach:** v6.6.1 patch release once entry points are tested on those pages.
